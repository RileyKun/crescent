...
// vec3_t are pitch (around y), Yaw (around Z), Roll (around X)
// vec3_t are exponetial maps (an axis in HL units scaled by a "twist" angle in degrees)
//		They can be transformed like normals/covec3_ts and added linearly
...
public:
	...
	virtual CPhysConvex * ConvexFromVerts(vec3_t * *pVerts, int vertCount) = 0;
	...
	virtual CPhysConvex* BBoxToConvex(const vec3_t& mins, const vec3_t& maxs) = 0;
	...
	virtual void			ConvexesFromConvexPolygon(const vec3_t& vPolyNormal, const vec3_t* pPoints, int iPointCount, CPhysConvex** pOutput) = 0;
	...
	virtual void			PolysoupAddTriangle(CPhysPolysoup* pSoup, const vec3_t& a, const vec3_t& b, const vec3_t& c, int materialIndex7bits) = 0;
	...
	virtual vec3_t			CollideGetExtent(const CPhysCollide* pCollide, const vec3_t& collideOrigin, const vec3_t& collideAngles, const vec3_t& direction) = 0;

	// Get an AABB for an oriented collision model
	virtual void			CollideGetAABB(vec3_t* pMins, vec3_t* pMaxs, const CPhysCollide* pCollide, const vec3_t& collideOrigin, const vec3_t& collideAngles) = 0;

	virtual void			CollideGetMassCenter(CPhysCollide* pCollide, vec3_t* pOutMassCenter) = 0;
	virtual void			CollideSetMassCenter(CPhysCollide* pCollide, const vec3_t& massCenter) = 0;
	...
	virtual vec3_t			CollideGetOrthographicAreas(const CPhysCollide* pCollide) = 0;
	virtual void			CollideSetOrthographicAreas(CPhysCollide* pCollide, const vec3_t& areas) = 0;
	...
	virtual CPhysCollide* BBoxToCollide(const vec3_t& mins, const vec3_t& maxs) = 0;
	...
	virtual void TraceBox(const vec3_t& start, const vec3_t& end, const vec3_t& mins, const vec3_t& maxs, const CPhysCollide* pCollide, const vec3_t& collideOrigin, const vec3_t& collideAngles, trace_t* ptr) = 0;
	virtual void TraceBox(const Ray_t& ray, const CPhysCollide* pCollide, const vec3_t& collideOrigin, const vec3_t& collideAngles, trace_t* ptr) = 0;
	virtual void TraceBox(const Ray_t& ray, unsigned int contentsMask, IConvexInfo* pConvexInfo, const CPhysCollide* pCollide, const vec3_t& collideOrigin, const vec3_t& collideAngles, trace_t* ptr) = 0;

	// Trace one collide against another
	virtual void TraceCollide(const vec3_t& start, const vec3_t& end, const CPhysCollide* pSweepCollide, const vec3_t& sweepAngles, const CPhysCollide* pCollide, const vec3_t& collideOrigin, const vec3_t& collideAngles, trace_t* ptr) = 0;

	// relatively slow test for box vs. truncated cone
	virtual bool			IsBoxIntersectingCone(const vec3_t& boxAbsMins, const vec3_t& boxAbsMaxs, const truncatedcone_t& cone) = 0;
	...
	virtual int				CreateDebugMesh(CPhysCollide const* pCollisionModel, vec3_t** outVerts) = 0;
	// destroy the list of verts created by CreateDebugMesh
	virtual void			DestroyDebugMesh(int vertCount, vec3_t* outVerts) = 0;
...
public:
	...
	virtual void	GetTriangleVerts(int convexIndex, int triangleIndex, vec3_t* verts) = 0;

	// UNDONE: This doesn't work!!!
	virtual void	SetTriangleVerts(int convexIndex, int triangleIndex, const vec3_t* verts) = 0;
...
public:
	...
	virtual bool VehiclePointInWater(const vec3_t& vecPoint) = 0;
};
...
{
public:
	virtual void GetSurfaceNormal(vec3_t & out) = 0;		// normal points toward second object (object index 1)
	virtual void GetContactPoint(vec3_t& out) = 0;		// contact point of collision (in world space)
	virtual void GetContactSpeed(vec3_t& out) = 0;		// speed of surface 1 relative to surface 0 (in world space)
};
...
public:
	...
	virtual void Update(const vec3_t & position, const vec3_t & angles, float timeOffset) = 0;
	...
	virtual void GetLastImpulse(vec3_t* pOut) = 0;
	...
	virtual float GetTargetPosition(vec3_t* pPositionOut, vec3_t* pAnglesOut) = 0;
...
public:
	...
	virtual simresult_e	Simulate(IPhysicsMotionController* pController, IPhysicsObject* pObject, float deltaTime, vec3_t& linear, vec3_t& angular) = 0;
};
...
public:
	...
	// gravity is a 3-vec3_t in in/s^2
	virtual void			SetGravity(const vec3_t& gravityvec3_t) = 0;
	virtual void			GetGravity(vec3_t* pGravityvec3_t) const = 0;
	...
	virtual IPhysicsObject* CreatePolyObject(const CPhysCollide* pCollisionModel, int materialIndex, const vec3_t& position, const vec3_t& angles, objectparams_t* pParams) = 0;
	// same as above, but this one cannot move or rotate (infinite mass/inertia)
	virtual IPhysicsObject* CreatePolyObjectStatic(const CPhysCollide* pCollisionModel, int materialIndex, const vec3_t& position, const vec3_t& angles, objectparams_t* pParams) = 0;
	// Create a perfectly spherical object
	virtual IPhysicsObject* CreateSphereObject(float radius, int materialIndex, const vec3_t& position, const vec3_t& angles, objectparams_t* pParams, bool isStatic) = 0;
	...
	virtual void SweepCollideable(const CPhysCollide* pCollide, const vec3_t& vecAbsStart, const vec3_t& vecAbsEnd,
		const vec3_t& vecAngles, unsigned int fMask, IPhysicsTraceFilter* pTraceFilter, trace_t* pTrace) = 0;
...
public:
	...
	virtual vec3_t			GetInertia(void) const = 0;
	virtual vec3_t			GetInvInertia(void) const = 0;
	virtual void			SetInertia(const vec3_t& inertia) = 0;
	...
	virtual vec3_t			GetMassCenterLocalSpace() const = 0;

	// NOTE: This will teleport the object
	virtual void			SetPosition(const vec3_t& worldPosition, const vec3_t& angles, bool isTeleport) = 0;
	virtual void			SetPositionMatrix(const matrix3x4_t& matrix, bool isTeleport) = 0;

	virtual void			GetPosition(vec3_t* worldPosition, vec3_t* angles) const = 0;
	...
	virtual void			SetVelocity(const vec3_t* velocity, const vec3_t* angularVelocity) = 0;

	// like the above, but force the change into the simulator immediately
	virtual void			SetVelocityInstantaneous(const vec3_t* velocity, const vec3_t* angularVelocity) = 0;

	// NOTE: velocity is in worldspace, angularVelocity is relative to the object's 
	// local axes (just like pev->velocity, pev->avelocity)
	virtual void			GetVelocity(vec3_t* velocity, vec3_t* angularVelocity) const = 0;

	// NOTE: These are velocities, not forces.  i.e. They will have the same effect regardless of
	// the object's mass or inertia
	virtual void			AddVelocity(const vec3_t* velocity, const vec3_t* angularVelocity) = 0;
	// gets a velocity in the object's local frame of reference at a specific point
	virtual void			GetVelocityAtPoint(const vec3_t& worldPosition, vec3_t* pVelocity) const = 0;
	// gets the velocity actually moved by the object in the last simulation update
	virtual void			GetImplicitVelocity(vec3_t* velocity, vec3_t* angularVelocity) const = 0;
	...
	virtual void			LocalToWorld(vec3_t* worldPosition, const vec3_t& localPosition) const = 0;
	virtual void			WorldToLocal(vec3_t* localPosition, const vec3_t& worldPosition) const = 0;

	// transforms a vec3_t (no translation) from object-local to world space
	virtual void			LocalToWorldvec3_t(vec3_t* worldvec3_t, const vec3_t& localvec3_t) const = 0;
	// transforms a vec3_t (no translation) from world to object-local space
	virtual void			WorldToLocalvec3_t(vec3_t* localvec3_t, const vec3_t& worldvec3_t) const = 0;

	// push on an object
	// force vec3_t is direction & magnitude of impulse kg in / s
	virtual void			ApplyForceCenter(const vec3_t& forcevec3_t) = 0;
	virtual void			ApplyForceOffset(const vec3_t& forcevec3_t, const vec3_t& worldPosition) = 0;
	// apply torque impulse.  This will change the angular velocity on the object.
	// HL Axes, kg degrees / s
	virtual void			ApplyTorqueCenter(const vec3_t& torque) = 0;
	...
	virtual void			CalculateForceOffset(const vec3_t& forcevec3_t, const vec3_t& worldPosition, vec3_t* centerForce, vec3_t* centerTorque) const = 0;
	// Calculates the linear/angular velocities on the center of mass for an offset force impulse (pass output to AddVelocity)
	virtual void			CalculateVelocityOffset(const vec3_t& forcevec3_t, const vec3_t& worldPosition, vec3_t* centerVelocity, vec3_t* centerAngularVelocity) const = 0;
	// calculate drag scale
	virtual float			CalculateLinearDrag(const vec3_t& unitDirection) const = 0;
	virtual float			CalculateAngularDrag(const vec3_t& objectSpaceRotationAxis) const = 0;
	...
	virtual bool			GetContactPoint(vec3_t* contactPoint, IPhysicsObject** contactObject) const = 0;
	...
	virtual void			UpdateShadow(const vec3_t& targetPosition, const vec3_t& targetAngles, bool tempDisableGravity, float timeOffset) = 0;

	// returns number of ticks since last Update() call
	virtual int				GetShadowPosition(vec3_t* position, vec3_t* angles) const = 0;
...
public:
	...
	virtual void			GetEndpoints(vec3_t * worldPositionStart, vec3_t * worldPositionEnd) = 0;
...
public:
	...
	virtual void	GetSurfacePlane(vec3_t* pNormal, float* pDist) const = 0;
...
struct fluidparams_t
{
	vec4_t	surfacePlane;	// x,y,z normal, dist (plane constant) fluid surface
	vec3_t		currentVelocity; // velocity of the current in inches/second
