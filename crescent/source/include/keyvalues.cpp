...
bool KeyValues::LoadFromBuffer(char const* resource_name, const char* buffer, void* file_system, const char* path_id) {
	return ( ( int(__thiscall*)(KeyValues*, char const*, const char*, void*, const char*) )offsets::kv_load_from_buffer )(this, resource_name, buffer, file_system, path_id);
}

void KeyValues::Initialize(char* name)
{
	( ( KeyValues * (__thiscall*)(KeyValues*, char*) )offsets::kv_initialize )( this, name );
}
...
KeyValues::KeyValues(const char* name)
{
	...
	Initialize((char*)_name);
}

KeyValues* KeyValues::FindKey(const char* keyName, bool bCreate)
{
	return ( ( KeyValues * (__thiscall*)(KeyValues*, const char*, bool) )offsets::kv_find_key )( this, keyName, bCreate );
}
...
clr_t KeyValues::GetColor(const char* keyName)
{
	clr_t color{ 0, 0, 0, 0 };
	KeyValues* dat = FindKey(keyName, false);
	if (dat)
...
void KeyValues::SetColor(const char* keyName, clr_t value)
{
	KeyValues* dat = FindKey(keyName, true);
